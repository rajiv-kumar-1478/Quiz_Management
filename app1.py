...
def reset_all_students(quiz_id):
    try:
        ...
        db.session.rollback()
        flash(f"⚠️ Error resetting all student attempts: {str(e)}", "danger")
...
def take_quiz(quiz_id):
    ...
    # Randomize question order or load existing
    question_ids = list(quiz_session.saved_answers.keys()) if quiz_session.saved_answers else \
                   [q.id for q in random.sample(all_questions, total_questions)]
